...
def net_pairs(all_pads, refs, targets):
    ...
    for rec in all_pads:
        ...
        if ref not in refs:
            continue
        cands = [
            (math.hypot(x - tx, y - ty), tr, tp)
            for tr, tp, tx, ty in by_net[net]
            if tr in targets
        ]
        if not cands:
            continue
...
def drc_electrical():
    ...
    viol = sum(1 for v in d.get("violations", []) if v["type"] in ELECTRICAL_DRC)
...
def check_rules(board, ox, oy, iso, non):
    ...
    for ref in sorted({a[0] for a in iso} & {c[0] for c in non}):
        f = next((q for q in board.Footprints() if q.GetReference() == ref), None)
        if f is None:
            continue
    ...
    for a in iso:
        iso_layers |= a[4]
    shared = [c[0] for c in non if c[0] in PACK_REFS and (c[4] & iso_layers)]
    return {
...
def regularity(board, ox, oy):
    ...
    aligned += sum(1 for v in xs if sum(1 for w in xs if abs(v - w) <= TOL_MM) > 1)
    return {
...
def main() -> int:
    ...
    obb = ob.BBox()
    box = (
        pcbnew.ToMM(obb.GetLeft() - ox),
        pcbnew.ToMM(obb.GetRight() - ox),
        pcbnew.ToMM(obb.GetTop() - oy),
        pcbnew.ToMM(obb.GetBottom() - oy),
    )
    creep = min(
        (nearest(x, non, box)[0] for x in iso if nearest(x, non, box) is not None),
        default=999.0,
    )
    ...
    if not a.json:
        ...
        print(
            f"   creepage min       {e['creepage_min_mm']:6.2f} mm  "
            f"(need {CREEPAGE_REQ_MM}) {'OK' if e['creepage_ok'] else 'FAIL'}",
            file=sys.stderr,
        )
        print(
            f"   isolated lead max  {e['iso_lead_max_mm']:6.2f} mm  "
            f"(need <={ISO_LEAD_MAX_MM}) "
            f"{'OK' if e['iso_lead_ok'] else 'FAIL'}  {e['iso_lead_worst']}",
            file=sys.stderr,
        )
        print(
            f"   gate loop max      {e['gate_loop_max_mm']:6.2f} mm  "
            f"{e['gate_loop_worst']}",
            file=sys.stderr,
        )
        print(
            f"   commutation max    {e['commutation_max_mm']:6.2f} mm  "
            f"{e['commutation_worst']}",
            file=sys.stderr,
        )
        print(f"   DRC electrical     {e['drc_electrical']:6d}", file=sys.stderr)
        print(
            f"   unconnected        {e['unconnected']:6d}"
        ...
        print(
            f"   distinct rows/cols {r['distinct_rows']:3d} / "
            f"{r['distinct_cols']:3d}  of {r['footprints']} parts",
            file=sys.stderr,
        )
        print(f"   off-row residual   {r['row_residual_mm']:6.3f} mm", file=sys.stderr)
        print(f"   sharing a row/col  {r['alignment_frac']:6.1%}", file=sys.stderr)
        k = score["rules"]
        print("RULES (isolated-interface boards)", file=sys.stderr)
        print(
            f"   isolator perpendicular to the long axis: "
