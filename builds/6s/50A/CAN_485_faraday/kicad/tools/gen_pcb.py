...
def two_pad_smd(lib_id, ref, value, span, pad_w, pad_h):
    ...
    fp.pads = [
        Pad(
            ...
            shape="roundrect",
            roundrectRatio=0.2,
            position=Position(-span / 2, 0, 0),
            size=Position(pad_w, pad_h),
...
def two_pad_tht(lib_id, ref, value, pitch, drill=0.8, pad_d=1.6):
    ...
    fp.pads = [
        Pad(
            ...
            type="thru_hole",
            shape="rect",
            position=Position(-pitch / 2, 0, 0),
            size=Position(pad_d, pad_d),
...
def dual_row_smd(lib_id, ref, value, npins, row_span, pitch, pad_w, pad_h):
    ...
    n_side = npins // 2
    y0 = -(n_side - 1) * pitch / 2
    fp.graphicItems = fp_text_pair(
        ref, value, size=1.0, dy=(n_side - 1) * pitch / 2 + 2
    )
    pads = []
...
def row_tht(lib_id, ref, value, npins, pitch, drill=1.0, pad_d=1.7):
    ...
    fp.tstamp = U()
    fp.graphicItems = fp_text_pair(
        ref, value, size=1.0, dy=(npins - 1) * pitch / 2 + 2.5
    )
    pads = []
...
def mechanical_outline(lib_id, ref, value, outer_w, outer_h, note):
    ...
    fp.attributes = Attributes(
        type="smd", excludeFromPosFiles=True, excludeFromBom=False
    )
    fp.tstamp = U()
    fp.graphicItems = fp_text_pair(ref, value, size=1.0, dy=outer_h / 2 + 2)
    fp.graphicItems.append(
        FpRect(
            start=Position(-outer_w / 2, -outer_h / 2),
    ...
    fp.graphicItems.append(
        FpText(
            ...
            layer="Cmts.User",
            effects=Effects(font=Font(height=0.8, width=0.8)),
            tstamp=U(),
        )
    )
    fp.pads = [
        Pad(
            ...
            type="smd",
            shape="rect",
            position=Position(0, outer_h / 2 - 1.5, 0),
            size=Position(3, 2),
            layers=["F.Cu", "F.Paste", "F.Mask"],
            tstamp=U(),
        )
...
def reserved_area(x, y, w, h, text):
    ...
    items = [
        GrRect(
            start=Position(x - w / 2, y - h / 2),
            end=Position(x + w / 2, y + h / 2),
            layer="Cmts.User",
            width=0.15,
            tstamp=U(),
        ),
...
def build_footprint(ref, lib_id, value, footprint):
    if footprint == "Capacitor_THT:CP_Radial_D10.0mm_P5.00mm":
        ...
        or footprint == "Capacitor_SMD:C_0805_2012Metric"
    ):
...
def title_block():
    ...
    tb.comments[3] = (
        "Footprints placed from the populated schematic BOM; not yet netlisted or routed"
    )
    tb.comments[4] = (
        "U5 (DRV8353S) has no confirmed footprint (custom WQFN, size before fab) -- "
...
def build_and_write(out_path):
    ...
    for sym in sch.schematicSymbols:
        ...
        if ref == "U5":
            ...
            skipped.append(
                (
                    ref,
                    "reserved area only, no footprint fabricated -- see kicad/README.md",
                )
            )
            continue
        if ref in ("SH1", "SH2"):
    ...
    x, y = REF_POS["J5"]
    fp = two_pad_tht(
        "Connector_Wire:SolderWirePad_1x02_P5.08mm_Drill1.3mm",
        "J5",
        "BATT_IN",
        pitch=5.08,
        drill=1.3,
    ...
    board.graphicItems.append(
        GrRect(
            start=Position(ox0, oy0),
            end=Position(ox1, oy1),
            layer="Edge.Cuts",
            width=0.15,
            tstamp=U(),
        )
    ...
    print(
        f"board outline {board_w:.1f}x{board_h:.1f}mm, centered on A3 page (translate {tx:.2f},{ty:.2f})"
    )
